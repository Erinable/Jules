...
class CodeVersionRepository:
    ...
    def create(self, file_id: uuid.UUID, content: str, version_number: int, commit_hash: Optional[str] = None) -> CodeVersion:
        """
        创建新的代码版本记录
        ...
        Returns:
            CodeVersion: 创建的版本对象
        """
        version = CodeVersion(file_id=file_id, content=content, version_number=version_number, commit_hash=commit_hash)
        self.db.add(version)
        self.db.commit()
    ...
    def get_latest(self, file_id: uuid.UUID) -> Optional[CodeVersion]:
        ...
        """
        return self.db.query(CodeVersion).filter(CodeVersion.file_id == file_id).order_by(CodeVersion.version_number.desc()).first()
    ...
    def get_history(self, file_id: uuid.UUID, limit: int = 10) -> list[CodeVersion]:
        ...
        """
        return self.db.query(CodeVersion).filter(CodeVersion.file_id == file_id).order_by(CodeVersion.version_number.desc()).limit(limit).all()
    ...
    def get_by_version_number(self, file_id: uuid.UUID, version_number: int) -> Optional[CodeVersion]:
        ...
        Returns:
            Optional[CodeVersion]: 版本对象，不存在则返回 None
        """
        return self.db.query(CodeVersion).filter(CodeVersion.file_id == file_id, CodeVersion.version_number == version_number).first()
    ...
    def get_all(self, limit: int = 100, offset: int = 0) -> list[CodeVersion]:
        ...
        """
        return self.db.query(CodeVersion).order_by(CodeVersion.created_at.desc()).limit(limit).offset(offset).all()
