...
class AgentExecutionRepository:
    ...
    def create(self, task_id: uuid.UUID, agent_type: str, state: Optional[dict[str, Any]] = None) -> AgentExecution:
        """
        创建新的 Agent 执行记录
        ...
        execution = AgentExecution(
            task_id=task_id, agent_type=agent_type, state=state or {}, status="running", started_at=datetime.utcnow()
        )
    ...
    def get_by_task(self, task_id: uuid.UUID) -> list[AgentExecution]:
        ...
        """
        return self.db.query(AgentExecution).filter(AgentExecution.task_id == task_id).order_by(AgentExecution.started_at.desc()).all()
    ...
    def get_all(self, limit: int = 100, offset: int = 0) -> list[AgentExecution]:
        ...
        Returns:
            list[AgentExecution]: 执行记录列表
        """
        return self.db.query(AgentExecution).order_by(AgentExecution.started_at.desc()).limit(limit).offset(offset).all()

    def update_status(self, execution_id: uuid.UUID, status: str, state: Optional[dict[str, Any]] = None) -> bool:
        """
        更新执行状态（数据库操作）
