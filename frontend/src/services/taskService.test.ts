...
describe('taskService', () => {
  ...
  describe('getAll', () => {
    it('fetches all tasks without filters', async () => {
      const mockTasks = [
        { id: '1', project_id: 'p1', title: 'Task 1', status: 'pending', priority: 5, created_at: '2024-01-01' },
      ]
    ...
    it('fetches tasks with filters', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/tasks/', { params: { status: 'completed', priority: 10 } })
    })
  })

  describe('getById', () => {
    it('fetches a task by ID', async () => {
      const mockTask = { id: '1', project_id: 'p1', title: 'Task 1', status: 'pending', priority: 5, created_at: '2024-01-01' }
  ...
  describe('create', () => {
    it('creates a new task', async () => {
      ...
      const createdTask = { id: '2', ...newTask, status: 'pending', created_at: '2024-01-01' }
  ...
  describe('update', () => {
    it('updates a task', async () => {
      ...
      const updatedTask = { id: '1', project_id: 'p1', title: 'Updated Title', status: 'pending', priority: 5, created_at: '2024-01-01' }
  ...
  describe('updateStatus', () => {
    it('updates task status', async () => {
      ...
      const updatedTask = { id: '1', project_id: 'p1', title: 'Task', status: 'completed' as const, priority: 5, created_at: '2024-01-01' }
      vi.mocked(apiClient.patch).mockResolvedValue({ data: updatedTask })

      const result = await taskService.updateStatus('1', statusUpdate)

      expect(apiClient.patch).toHaveBeenCalledWith('/tasks/1/status', statusUpdate)
      expect(result).toEqual(updatedTask)
    })
  })
