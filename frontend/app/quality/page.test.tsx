...
describe("QualityPage", () => {
  ...
  it("displays error message on fetch failure", async () => {
    vi.mocked(qualityService.getLatest).mockRejectedValue(
      new Error("API Error"),
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Failed to load quality metric"),
      ).toBeInTheDocument();
    });
  });

  it("shows loading spinner while fetching", () => {
    vi.mocked(qualityService.getLatest).mockImplementation(
      () => new Promise(() => {}),
    );
    vi.mocked(qualityService.getHistory).mockResolvedValue([]);
