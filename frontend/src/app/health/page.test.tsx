...
describe("HealthPage", () => {
  ...
  it("displays error message on fetch failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Failed to load health status"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows loading spinner while fetching", () => {
    vi.mocked(healthService.getHealth).mockImplementation(
      () => new Promise(() => {}),
    );
    vi.mocked(healthService.getReady).mockImplementation(
      () => new Promise(() => {}),
    );
    vi.mocked(healthService.getLive).mockImplementation(
      () => new Promise(() => {}),
    );
