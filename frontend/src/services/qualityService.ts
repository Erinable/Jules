...
import type { QualityMetric, QualityMetricCreate, QualityMetricResponse, PaginationParams } from '@/types'
...
export const qualityService = {
  ...
  async create(metric: QualityMetricCreate): Promise<QualityMetricResponse> {
    const response = await apiClient.post<QualityMetricResponse>('/quality/', metric)
    return response.data
  },
  ...
  async getLatest(projectId: string): Promise<QualityMetric> {
    const response = await apiClient.get<QualityMetric>(`/quality/project/${projectId}/latest`)
    return response.data
  },
   ...
   */
  async getHistory(projectId: string, params?: PaginationParams): Promise<QualityMetric[]> {
    const response = await apiClient.get<QualityMetric[]>(`/quality/project/${projectId}/history`, { params })
    return response.data
  },
